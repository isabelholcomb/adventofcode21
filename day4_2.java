...
public class day4 {
	...
	public static void main(String[] args) throws Exception {
		File file = new File("C:\\Users\\isabel\\Downloads\\aocd4.txt");
    	...
    	int winningCard = -1;
    	boolean winners[] = new boolean[cardCounter]; //mark which cards win
    	int numWinners = 0; //keeps track of how many winners there are
    	int losingCard = -1;
	    				...
	    				for(int k = 0; k<5; ++k)
	    				{
	    					if (currentNumber == stackOfCards[j][k][i])
	    					{
	    						...
	    						if(checkForWinner(tf[i]))
		    					{
		    						...
		    						//check if it's the second to last winning card
		    						if(!winners[winningCard])
		    						{
		    							winners[winningCard] = true;
		    							++numWinners;
		    							if (cardCounter - numWinners == 1)
		    							{
		    								
		    						    	for(int m = 0; m<cardCounter;++m)
		    						    	{
		    						    		if(!winners[m])
		    						    		{
		    						    			losingCard = m;
    	...
    	for(int j = 0; j<5; ++j)
		{
			for(int k = 0; k<5; ++k)
			{
				if(!tf[losingCard][j][k])
				{
					sum += stackOfCards[j][k][losingCard];
				}
			}
		}
